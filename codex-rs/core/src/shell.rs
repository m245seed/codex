...
impl Shell {
    pub fn format_default_shell_invocation(&self, command: Vec<String>) -> Option<Vec<String>> {
        match self {
            Shell::Zsh(shell) | Shell::Bash(shell) => {
                format_shell_invocation_with_rc(&command, &shell.shell_path, &shell.rc_path)
            }
            Shell::PowerShell(ps) => self.format_powershell_invocation(command, ps),
            Shell::Unknown => None,
        }
    }

    fn format_powershell_invocation(&self, command: Vec<String>, ps: &PowerShellConfig) -> Option<Vec<String>> {
        if let Some(script) = strip_bash_lc(&command) {
            return match &ps.bash_exe_fallback {
                Some(bash) => Some(vec![bash.to_string_lossy().to_string(), "-lc".to_string(), script]),
                None => Some(vec![ps.exe.clone(), "-NoProfile".to_string(), "-Command".to_string(), script]),
            };
        }

        if command.first().map(String::as_str) != Some(&ps.exe) {
            if command.iter().any(|a| a.contains('\n') || a.contains('\r')) {
                return Some(command);
            }
            return shlex::try_join(command.iter().map(String::as_str)).ok()
                .map(|arg| vec![ps.exe.clone(), "-NoProfile".to_string(), "-Command".to_string(), arg]);
        }
    ...
    pub fn name(&self) -> Option<String> {
        match self {
            Shell::Zsh(shell) | Shell::Bash(shell) => {
                std::path::Path::new(&shell.shell_path)
                    .file_name()
    ...
    unsafe {
        ...
        if !pw.is_null() {
            ...
            if shell_path.ends_with("/zsh") {
                return Shell::Zsh(UnixShell {
                    shell_path,
                    rc_path: format!("{home_path}/.zshrc"),
                });
            }

            if shell_path.ends_with("/bash") {
                return Shell::Bash(UnixShell {
                    shell_path,
                    rc_path: format!("{home_path}/.bashrc"),
                });
            }
        }
    }
...
mod tests {
    ...
    async fn test_run_with_profile_bashrc_not_exists() {
        let shell = Shell::Bash(UnixShell {
            shell_path: "/bin/bash".to_string(),
            rc_path: "/does/not/exist/.bashrc".to_string(),
        });
        let actual_cmd = shell.format_default_shell_invocation(vec!["myecho".to_string()]);
        assert_eq!(
            actual_cmd,
